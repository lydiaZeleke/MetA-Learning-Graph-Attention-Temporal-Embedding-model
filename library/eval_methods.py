...
from library.spot import SPOT, dSPOT
...
def calc_point2point(predict, actual):
    ...
    actual = actual.numpy()
    actual = actual.squeeze()  # from (N, 1) to (N,)

    # Convert both to the same type, for example boolean:
    actual = actual.astype(bool)   # Convert actual from int to bool if it’s 0/1
    predict = predict.astype(bool) # Ensure predict is also boolean

    # Now both have shape (N,) and type bool, so the operations are consistent
    TP = np.sum(predict & actual)
    TN = np.sum(~predict & ~actual)
    FP = np.sum(predict & ~actual)
    FN = np.sum(~predict & actual)
...
def evaluate_auc(y_true, anomaly_scores):
    """
    Compute AUC-ROC and AUC-PR for anomaly detection.
    
    :param y_true: Ground truth anomaly labels (1 = anomaly, 0 = normal)
    :param anomaly_scores: Model's anomaly scores (higher = more anomalous)
    :return: AUC-ROC and AUC-PR scores
    """
    # Compute AUC-ROC
