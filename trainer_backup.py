...
class Trainer:
    ...
    def generate_csv_log_writer(self, base_dir="meta_training_logs"):
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        support_log_path = f"{base_dir}/support_logs_{timestamp}.csv"
        query_log_path = f"{base_dir}/query_logs_{timestamp}.csv"
    ...
    def compute_loss(self, inputs, targets, recons, preds):
        ...
        return prediction_loss, recon_loss, prediction_loss + recon_loss
    ...
    def meta_train(self, tasks, num_epochs=5, adaptation_steps=1):
        ...
        for epoch in range(num_epochs):
            ...
            for task_id, (support_loader, query_loader) in tasks.items():
                with higher.innerloop_ctx(
                    ...
                    for _ in range(adaptation_steps):
                        support_loss_accum, support_pred_loss_accum, support_recon_loss_accum = 0.0, 0.0, 0.0
                    ...
                    query_loss_accum, query_pred_loss_accum, query_recon_loss_accum = 0.0, 0.0, 0.0
                    ...
                    # Save per-task losses
                    support_losses.append(support_loss_accum.item())
                    query_losses.append(query_loss_accum)
            ...
            # # ---- Apply weighting based on support losses ----
            # support_losses = torch.tensor(support_losses, device=self.device)
            # weights = support_losses / support_losses.sum()
            # weights = torch.clamp(weights, min=0.3, max=3.0)

            # meta_loss = 0.0
            # for i in range(len(query_losses)):
            #     meta_loss += weights[i] * query_losses[i]
    ...
    def meta_test(self, anomaly_det_args, test_tasks, label_tasks, adaptation_steps=1):
        ...
        task_label_len = [len(label_tasks[task_id][1].dataset.data) for task_id in label_tasks]
        task_test_len = [len(test_tasks[task_id][1].dataset.data) for task_id in test_tasks]
        for task_idx, (task_id, (support_loader, query_loader)) in enumerate(test_tasks.items()):
            ...
            if task_id in [13, 19, 23, 29, 36, 66, 71, 82, 87]:
                ...
                for _ in range(adaptation_steps):
                    ...
                    for (x_sup, y_sup) in support_loader:
                        ...
                        if y_sup.ndim == 3:
                            y_sup = y_sup.squeeze(1)
                        ############# 
                    support_loss_test, _, _ = self.compute_loss(x_sup, y_sup, recons_sup, preds_sup)
