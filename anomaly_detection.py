...
class AnomalyDetector:
    ...
    def predict_anomalies(self, train, test, true_anomalies, load_scores=False, save_output=True, 
        ...
        # Example usage:
        # y_test = Ground truth anomaly labels (binary)
        # anomaly_scores = Predicted anomaly scores from the model

        auc_roc, auc_pr = evaluate_auc(true_anomalies, test_anomaly_scores)
    ...
    def plot_anomaly_detection_results(self, df, num_features=2, max_subplots_per_figure=3):
        ...
        :param df: Pandas DataFrame containing forecast, reconstruction, true values, and anomaly scores.
        :param anomaly_interpretation_file: CSV file containing encounter-wise corrupted fields.
        :param num_features: Number of features to plot.
        :param max_subplots_per_figure: Maximum number of subplots per figure.
        """
        anomaly_interpretation_file = "./datasets/CUSTOM/anomaly_interpretation.csv"

        # Load the anomaly interpretation data
        enc_corruption_df = pd.read_csv(anomaly_interpretation_file)
        enc_corruption_df["corrupted_fields"] = enc_corruption_df["corrupted_fields"].apply(lambda x: x.split(", "))
        ...
        for key in corrupted_fields_dict:
            if isinstance(corrupted_fields_dict[key], list):  # Ensure it's a list before processing
                corrupted_fields_dict[key] = [field.strip() for item in corrupted_fields_dict[key] for field in item.split(',')]

        # Extract time steps
        time_steps = np.arange(len(df))

        # Divide plots into multiple figures
        figures = []  # Store figure objects to keep them open
        corrupted_masks = {}
        for fig_idx in range(0, num_features, max_subplots_per_figure):
            fig, axes = plt.subplots(max_subplots_per_figure, 2, figsize=(14, max_subplots_per_figure * 3))
            axes = axes.reshape(-1, 2)  # Ensure axes are always a 2D array
            
            for i, feature_idx in enumerate(range(fig_idx, min(fig_idx + max_subplots_per_figure, num_features))):
                feature_name = f"True_{feature_idx}"
                forecast_name = f"Forecast_{feature_idx}"
                recon_name = f"Recon_{feature_idx}"
                anomaly_score_name = f"A_Score_{feature_idx}"
                pred_anomaly_name = f"A_Pred_{feature_idx}"

                # Extract relevant columns
                true_values = df[feature_name]
                forecasted = df[forecast_name]
                reconstructed = df[recon_name]
                anomaly_scores = df[anomaly_score_name]
                ...
                # Identify predicted anomalies (highlight in second plot)
                predicted_anomalies = df[pred_anomaly_name] == 1 if pred_anomaly_name in df.columns else np.zeros_like(anomaly_scores, dtype=bool)
                ...
                y_min, y_max = min(true_values), max(true_values)
                if y_min == y_max:  # Handle cases where all values are the same (e.g., all zeros)
                    y_min -= 0.1  # Small downward shift
                    y_max += 0.1  # Small upward shift


                # Highlight true anomalies only on corrupted fields
                ax1.fill_between(time_steps, y_min, y_max, where=corrupted_mask, color='lightgreen', alpha=0.5, label="Corrupted Fields")
                
                ax1.set_title(f"{feature_labels[feature_idx]}: Timeseries & Predictions")
                ax1.legend()
                ax1.set_xlabel("Time Step")
                ax1.set_ylabel("Value")
            ...
            plt.tight_layout()
            figures.append(fig)  # Store figure object to keep it open
