...
class TemporalConvBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        out = out + residual  # Replaced out += residual because it was causing inplace errors in meta-gradient update s
...
class GraphAttentionNetworkMultiHead(nn.Module):
    def __init__(self, 
                 num_nodes, 
                 node_embedding_dim,    
                 dropout, 
        ...
        use_bias: If True, include a bias term in the attention mechanism
        """
        super(GraphAttentionNetworkMultiHead, self).__init__()
        self.num_nodes = num_nodes
        self.node_embedding_dim = node_embedding_dim
        self.dropout = dropout
        self.use_gatv2 = use_gatv2
...
class VAE(nn.Module):
    """
    VAE for reconstructing a single feature vector from the GRU's last hidden state.

    :param hidden_dim: Dimension of the GRU's last hidden state.
    :param num_features: Dimension of the output vector to reconstruct.
    :param latent_dim: Dimension of the latent space.
    """

    def __init__(self, hidden_dim, num_features, latent_dim, dropout):
    ...
    def forward(self, h):
        """
        :param h: GRU's last hidden state of shape (batch_size, hidden_dim)
        :return: reconstructed (batch_size, num_features), mu, log_var
        """
        batch_size = h.size(0)
        ...
        # Decode to (batch_size, num_features)
...
class DecoderOnlyModel(nn.Module):
    """
    A simple decoder that maps a single vector of shape (batch_size, hidden_dim)
    or (batch_size, some_dim) into a sequence (batch_size, seq_len, num_features).
    """

    def __init__(self, seq_len, hidden_dim, num_features, dropout=0.1):
        super(DecoderOnlyModel, self).__init__()
        self.seq_len = seq_len
        self.hidden_dim = hidden_dim
        self.num_features = num_features

        # Example: a small MLP or RNN that goes from hidden_dim -> (seq_len * num_features)
        self.fc = nn.Sequential(
            nn.Linear(hidden_dim, 128),
            nn.ReLU(),
            nn.Dropout(dropout),
            nn.Linear(128, seq_len * num_features),
            nn.Sigmoid()  # or no activation, depending on your data scaling
        )
    ...
    def forward(self, h):
        ...
        """
        B = h.size(0)
        x = self.fc(h)  # (B, seq_len * num_features)
        out = x.view(B, self.seq_len, self.num_features)
        return out


####################
class VAE_GAT(nn.Module):
    """VAE for reconstructing GAT output
    :param final_hid_dim: Dimension of feature vectors from GAT output
    :param num_features: Number of features (nodes)
    :param latent_dim: Dimension of latent space
    """

    def __init__(self, node_embedding_dim, num_features, latent_dim):
        super(VAE, self).__init__()
        self.node_embedding_dim = node_embedding_dim
        self.num_features = num_features
        self.latent_dim = latent_dim

        # Encoder: Encode GAT output into latent space
        self.encoder = nn.Sequential(
            nn.Linear(node_embedding_dim * num_features, 256),
            nn.ReLU(),
            nn.Linear(256, 128),
            nn.ReLU()
        )
        self.mu = nn.Linear(128, latent_dim)        # Mean of latent space
        self.log_var = nn.Linear(128, latent_dim)  # Log variance of latent space

        # Decoder: Reconstruct GAT output from latent space
        self.decoder = nn.Sequential(
            nn.Linear(latent_dim, 128),
            nn.ReLU(),
            nn.Linear(128, 256),
            nn.ReLU(),
            nn.Linear(256, node_embedding_dim * num_features),
            nn.Sigmoid()  # Normalized output between 0 and 1
        )
    ...
    def forward(self, x):
        ...
        :param x: GAT output of shape (batch_size, final_hid_dim, num_features)
        :return: Reconstructed output, mean, and log variance
        """
        batch_size, final_hid_dim, num_features = x.size()

        # Flatten input for encoder
        x_flat = x.reshape(batch_size, -1)  # Shape: (batch_size, final_hid_dim * num_features)
        ...
        # Decode
        decoded = self.decoder(z)
        reconstructed = decoded.view(batch_size, final_hid_dim, num_features)  # Reshape to original GAT output shape
...
class GRUAT(nn.Module):
    ...
    alpha = 0.2, 
        ...
        self.forecasting_model = Forecasting_Model(gru_hid_dim, forecast_hid_dim, output_size, forecast_n_layers, dropout)
        self.vae = VAE_UPDATED(window_size, node_embed_dim, gru_hid_dim, output_size, vae_latent_dim, dropout)
        #self.vae = VAE(gru_hid_dim, num_features, vae_latent_dim, dropout)
        # self.recon_model =  DecoderOnlyModel(window_size, gru_hid_dim, num_features, dropout)
        #self.recon_model = ReconstructionModel(window_size, gru_hid_dim, recon_hid_dim, output_size, recon_n_layers, dropout)
    ...
    def forward(self, x):
        ...
        forecast_pred = self.forecasting_model(h_end_pred)
