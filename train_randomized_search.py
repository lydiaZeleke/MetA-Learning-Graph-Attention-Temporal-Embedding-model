...
param_distributions = {
    'batch_size': [32, 64, 128],        # Typically 256+ can be large for GRU-based models
    'window_size': [50, 100],           # Reasonable for short vs. medium sequences
    'n_epochs': [30, 50, 100],          # Fewer than 30 often insufficient, >100 can be tested if time allows
    'init_lr': [1e-3, 3e-4, 1e-4],      # 1e-2 can be too big for many RNN/attention combos
    'dropout': [0.1, 0.2, 0.4],         # 0.3 was fine, but 0.4 often helps deeper or larger models
...
class GRUATModel(BaseEstimator):
    ...
    def fit(self, X, y=None):
        ...
        return train_and_evaluate_model(hyperparams)
    
    def score(self, X, y=None):
        # This should return a negative score for minimization (like negative MSE)
        loss = self.fit(X)
        return -loss  # Since lower loss is better
